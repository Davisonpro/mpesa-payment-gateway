        ...
        if (abs($difference) < 0.01) {
            ...
            $order->add_order_note(
                sprintf(
                    ...
                    __('M-Pesa payment received. Transaction ID: %1$s, Phone: %2$s', 'mpesa-payment-gateway'),
                    $transactionId,
                    $phone
                )
            );
            ...
            $order->add_order_note(
                sprintf(
                    ...
                    __('Partial M-Pesa payment received. Expected: %1$s, Received: %2$s, Shortage: %3$s. Transaction ID: %4$s', 'mpesa-payment-gateway'),
                    $orderTotal,
                    $amount,
                    $difference,
                    $transactionId
            ...
            $order->add_order_note(
                sprintf(
                    ...
                    __('M-Pesa payment received with overpayment. Expected: %1$s, Received: %2$s, Excess: %3$s. Transaction ID: %4$s', 'mpesa-payment-gateway'),
                    $orderTotal,
                    $amount,
                    abs($difference),
                    $transactionId
    ...
    private function processSuccessfulPayment($order, array $metadata): void
    {
        ...
        $order->add_order_note(
            sprintf(
                ...
                __('M-Pesa payment completed. Transaction ID: %1$s, Amount: %2$s, Phone: %3$s', 'mpesa-payment-gateway'),
                $transactionId,
                $amount,
                $phone
            )
    ...
    private function processFailedPayment($order, int $resultCode, string $resultDesc): void
    {
        ...
        $order->add_order_note(
            sprintf(
                ...
                __('M-Pesa payment failed. Code: %1$d, Message: %2$s', 'mpesa-payment-gateway'),
                $resultCode,
                $resultDesc
            )
        );
